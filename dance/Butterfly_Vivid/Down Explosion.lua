local t = Def.ActorFrame {
	...
	NOTESKIN:LoadActor( Var "Button", "Tap Explosion Bright" ) .. {
		...
		HoldingOffCommand=NOTESKIN:GetMetricA("GhostArrowBright", "HoldingOffCommand");
		RollOffCommand=NOTESKIN:GetMetricA("GhostArrowBright", "W1Command");
		JudgmentCommand=cmd(finishtweening);
		BrightCommand=cmd(visible,false);
		DimCommand=cmd(visible,true);
	};
	NOTESKIN:LoadActor( Var "Button", "HitMine Explosion" ) .. {
		InitCommand=cmd(zoomto,128,128;blend,"BlendMode_Add";diffusealpha,0);
